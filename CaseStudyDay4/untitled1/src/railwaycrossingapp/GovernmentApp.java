...
public class GovernmentApp {
	...
	void search(){
		RailwayCrossing railwayCrossing=null;
		System.out.println("Enter Crossing Name");
		String choise1 = scanner.nextLine();
		RailwayCrossing[] raliwayCrossingList = null;
		for( RailwayCrossing rc:raliwayCrossingList) {
			if(rc.getName().equalsIgnoreCase(choise1)) {
			rc=railwayCrossing;
			break;
			}
		}
	...
	RailwayCrossing[] raliwayCrossingList = null;
	for( RailwayCrossing rc:raliwayCrossingList) {
		if(rc.getName().equalsIgnoreCase(choise2)) {
		((Map<String, RailwayCrossing>) rc).remove(choise2);
		break;
		}
	}
	System.out.println(railwayCrossing);
